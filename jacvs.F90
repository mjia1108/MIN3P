...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/jacvs.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
!c ----------------------------------------------------------------------
      ...
      real*8 :: dvsflux, dvsstor, dqroot, qroot, qrootinc, totvsflux,  &
                vsstor, vsstorinc, vsfluxinc, fluxvs, storvs, rootwat, &
                hhinc, dtotvsflux, sa_eff_old
      integer :: chunk

      external storvs, fluxvs, fluxvs_dp, rhsvs, rootwat, soilparm
      real*8 :: fluxvs_dp

      real*8, parameter :: r0 = 0.0d0, r1 = 1.0d0, r2 = 2.0d0
      
      !temp hardwired parameters for dual permeability implementation
      !real*8 :: beta_dp, gamma_dp, dist_dp, cond_ex_sat, w_fracture,   &
      !          w_matrix
...
!c_trap set minimum effective saturation for trapped gas calculation
      ...
      !
      !  !cummj_dp  temp hardwired parameters for dual permeability implementation
      !  beta_dp = 3.0d0        !unitless
      !  gamma_dp = 0.4d0       !unitless
      !  dist_dp = 0.01         !m
      !  cond_ex_sat = 1.0d-4   !m day-1
      !  !cond_ex_sat = 0.0d0    !m day-1
      !  !w_fracture = 0.05      !unitless
      !  w_fracture = 0.5       !unitless        
      !
      !  !cummj_dp  precalculations for exchange terms
...
!c  flux calculations
          if (.not.dual_permeability) then
            ...
            vsflux(icon) = -fluxvs_dp(upstream,hhead(ivol),hhead(jvol),&
...
!#ifdef DEBUG
!          if(ivol > 0 .and. ivol < 10) then
!            write(idbg,'(3(a,1x,i4,1x),a,1x,l1,1x,2(a,1x),7(a,1x,e16.8,1x))') & 
...
!            write(idbg,'(a,1x,i4,1x,8(a,1x,e16.8,1x))') "-->jacvs A ivol",  &
...
!c  compute root water uptake for current control volume
      ...
      if (root_uptake) then
        ...
        qroot = cvol(ivol)*rootwat(sanew(ivol),                     &
     &                               mpropvs(ivol))
...
!#ifdef DEBUG
!        if(ivol > 0 .and. ivol < 10) then
!            write(idbg,'(a,1x,i4,1x,3(a,1x,e16.8,1x))') "-->jacvs B ivol",  &
...
!c  flux with incremented variables and derivative of flux
          ...
          if (.not.dual_permeability) then
            ...
            vsfluxinc = - fluxvs_dp(upstream,hhinc,hhead(jvol),        &
                                    uvsinc(ivol),uvsnew(jvol),         &
...
!                  "-->jacvs ivol",ivol,"jvol",jvol,"icon",icon,          &
!                  "upstream",upstream,"iups(icon)",iups(icon),           & 
!                  "hhinc",hhinc,"hhead(jvol)",hhead(jvol),"uvsinc(ivol)",&
!                  uvsinc(ivol),"uvsnew(jvol)",uvsnew(jvol),              &
!                  "relpinc(ivol)",relpinc(ivol),"relperm(jvol)",         &
!                  relperm(jvol),"cinfvs(i1)",cinfvs_a(i1)
!          end if
!#endif
...
!c  compute derivative of root water uptake for current control volume
      ...
      if (root_uptake) then
        ...
        qrootinc = cvol(ivol)*rootwat(sainc(ivol),                  &
     &                                  mpropvs(ivol))
