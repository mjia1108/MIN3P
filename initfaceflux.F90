...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/initfaceflux.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
!c  check if the two input volumes are linked and assign edge index
      do igb = 1,ngb_ijface
        ...
        if (ivol == jvol) then
          if (rank == 0) then  
            write(ilog,'(a)') 'ABNORMAL EXIT from initopgs-4'
            write(ilog,'(2a,3(a,1x,i6,1x))')                           &
                  'control volumes for interface ',                    &
                  'fluxes output are the same, ',                      &
                  'interface pair', igb,                               &
                  'ivol',ivol,'jvol',jvol                              
        ...
        if (ngb_vol_ijface_jtemp(igb) < 1) then
          if (rank == 0) then  
            write(ilog,*) 'ABNORMAL EXIT from initopgs-5'
            write(ilog,'(2a,3(a,1x,i6,1x))')                           &
                  'control volumes for interface ',                    &
...
500       continue   
        ...
        if (ngb_vol_ijface_area(igb) <= 0.0d0) then
          if (rank == 0) then
            write(ilog,*) 'Error: interfacial area is zero'
            close(ilog)
          end if
#ifdef PETSC
          call petsc_mpi_finalize
