...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/updtbcvs.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
!c ----------------------------------------------------------------------
      ...
      integer :: i, ibz, ierr, ivol, l_string, nbz
      real*8 :: areaf, tiny, dummy, xbmin, xbmax, ybmin, ybmax,        &
                zbmin, zbmax, tiny_time

      external checkerr, findstrg, findzone, readbloc, readzone

      parameter (tiny = 1.d-5, tiny_time=1.0d-10)
                  ...
                  if (btypezn.eq.'first') then
                    ...
                    if (pressure_head) then
                      if (.not. b_restart_update_bcvs) then
                        hhead(ivol) = rwork(ibz,1) + zg(ivol)
                      end if
                      bcondvs(nbvs) = rwork(ibz,1)
                    elseif (hydraulic_head) then
                      if (.not. b_restart_update_bcvs) then
                        hhead(ivol) = rwork(ibz,1)
                      end if
                      bcondvs(nbvs) = rwork(ibz,1) - zg(ivol)
                    end if
...
!c  time units
                    ...
                    bcondvs(nbvs) = areaf*rwork(ibz,1)*sec_per_days

                  elseif (btypezn.eq.'point') then

                    !bcondvs(ibvs) = rwork(ibz,1)*sec_per_days
                    bcondvs(nbvs) = rwork(ibz,1)*sec_per_days
