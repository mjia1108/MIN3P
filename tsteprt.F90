...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/tsteprt.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
!c  and actual update of current time step 
      ...
      if(rank == 0) then
        if(info_debug > 10) then
          write(idbg,'(a, 1x, e15.7)') "delt_rt-1: ", delt_rt
        end if
      end if
#endif
...
!c  take square root of factor for smoother changes
      ...
      if(rank == 0) then
        if(info_debug > 10) then
          write(idbg,'(a, 1x, e15.7)') "final delt_rt_iter after sqrt ", delt_rt_iter
        end if
      end if
#endif
...
!c  iterations
      ...
      if(rank == 0) then
        if(info_debug > 10) then
          write(idbg,'(a, 1x, e15.7)') "delt_rt-2: ", delt_rt
        end if
      end if
#endif
...
!c  volume fractions
      ...
      if(rank == 0) then
        if(info_debug > 10) then
          write(idbg,'(a, 1x, e15.7)') "delt_rt-3: ", delt_rt
        end if  
      end if
#endif
...
!c  apply upper and lower bounds for new time step
      ...
      if(rank == 0) then
        if(info_debug > 10) then
          write(idbg,'(a, 1x, e15.7)') "delt_rt-4: ", delt_rt
        end if
      end if
#endif
