...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/mbalrt.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
#include <finclude/petscsys.h>
      ...
      integer :: ierr, ivol, ibrt, istart ,iend, ic, i1, jvol, ibvs,   &
                 ix, ir, iaq, imb, ig, im, istop, ireac, jtemp
      ...
      real*8 :: absbalance, acoff, area_ivol, totvsflux, ddbdflux,     &
                ...
                gasp_m, gasd_m, gasv, relbalance, fluxv_vl_dp,         &
                fluxd_dp, diffcoff_g, gasdiff2, gasdiff2_s, fluxd_g_dp
      
      integer :: tid

      external acoff, bulkconc, bdryflux, comptotc, ddbdflux,         &
               diffcoff,fluxv_vl, fluxd, msysrt,                      &
               rateredx, totconcg, zero_r8, fluxv_vl_dp, fluxd_dp,    &
               diffcoff_g, gasdiff2, gasdiff2_s, fluxd_g_dp
     
      logical :: compute_diff

      real*8, parameter :: r0 = 0.0d0,r1=1.0d0,r100 = 100.0d0,         &
                           conv3 = 1.0d3, rtiny = 1.0d-10,             &
                           rverysmall = 1.0d-30,                       &
...
#endif  
      if (ng.gt.0) then
        call zero_r8(totgasdecay,n,1,1)
      end if
...
!c  flux contributions
    ...
    !$omp num_threads(numofthreads_global)                            &
    !$omp default(shared)                                             &
    !$omp private(i1, ibrt, ibvs, ic, iend, istart, ivol, jvol, so_av)&
...
!                              * cnew(ic,ivol) * Dzdeltac_loc/Dz2c_loc 
                    ...
                    if (.not.dual_permeability) then                    
                      ...
                      if (dabs(yg(ivol)-yg(jvol)).gt.rtiny) then 
                        totcflux(ic) = totcflux(ic) + conv3 *               & 
                                      (fluxv_vl_dp(totcnew(ic,ivol),        &
                                                totcnew(ic,jvol),           &
                                                ivol,jvol,                  &
                                                cinfrt_va(i1),ic)           &
                                    - fluxd_dp(totcnew(ic,ivol),            &
                                             totcnew(ic,jvol),cinfrt_da(i1),&
                                             cinfvs_a(i1),pornew(ivol),     &
                                             sanew(ivol),pornew(jvol),      &
                                             sanew(jvol),yg(ivol),yg(jvol)))
                      else
                        totcflux(ic) = totcflux(ic) + conv3 *               &
                                      (fluxv_vl(totcnew(ic,ivol),           &
                                                totcnew(ic,jvol),           &
...
!cprovi--------------------------------------------------------------------      
            ...
            !volume jvol is the volume connected to ivol in the y direction
            if(dual_permeability) then
              istart = iavs(ivol)+1 
              iend = iavs(ivol+1)-1
              do i1 = istart, iend
                jvol = javs(i1)
                if (dabs(yg(ivol)-yg(jvol)) > rtiny) then
                  exit  
                end if
              end do
              jtemp = i1
                ...
                if (b_fluxd_mixed_bcond(ivol)) then
                  ...
                  if (.not.dual_permeability) then  
                    ...
                    totcflux(ic) = conv3 *                             &
                                 ( totvsflux                           & !advective flux
                                 * bcondrt_a(ic,ibrt)                  &
                                 + fluxd_dp(totcnew(ic,ivol),          & !diffusive flux
                                         bcondrt_a(ic,ibrt),           &
                                         bdyinfrt_da,cinfvs_a(jtemp),  &
                  ...
                  totcflux(ic) = conv3 * (totvsflux*bcondrt_a(ic,ibrt))  ! advective flux only
                end if
...
!c new - diffusion expressed in terms of concentration gradients
     ...
     &                               cinfrt)                            ! cinfrt_dg(i1) -> cinfrt
                      ...
                      !gflux_ic = - fluxd_g_dp(totgnew(ic,ivol),              & !diffusion flux
                      !                        totgnew(ic,jvol),              &
                      !                        cinfrt,cinfvs_a(i1),           &
...
!c  mass loss due to first order gas decay
    ...
    !$omp num_threads(numofthreads_global)                            &
    !$omp default(shared)                                             &
    !$omp private (tid, ic, ig, ivol, gasdecayrate)                   &
...
!c  (inflow/outflow occurs only where first type boundary is specified)
    ...
    !$omp num_threads(numofthreads_global)                            &
    !$omp default(shared)                                             &
    !$omp private (i1, ibrt, istart, iend, ig, ivol, jvol, tid,       &
...
!c                   unique constant diffusion
     ...
     &                               cinfrt)                            ! cinfrt_dg(i1) -> cinfrt
                      ...
                      gflux_ig = - fluxd_g_dp(gnew(ig,ivol),                 & !diffusion flux
                                              gnew(ig,jvol),                 &
                                              cinfrt,cinfvs_a(i1),           &
                                              pornew(ivol),sgnew(ivol),      &
                                              pornew(jvol),sgnew(jvol),      &
...
!c  compute boundary influence coefficient
     ...
     &                               bdyinfrt_dg)) 
                ...
                totgflux(ig) = conv3 *                                       &
     &                       ( + fluxd_g_dp(gnew(ig,ivol),                   & !diffusive flux
     &                                      gbrt(ig,ibrt),                   &
     &                                      bdyinfrt_dg, cinfvs_a(jtemp),    &
     &                                      pornew(ivol),sgnew(ivol),        &
     &                                      pornew(jvol),sgnew(jvol),        &
