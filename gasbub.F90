...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/gasbub.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
#include <finclude/petscsys.h>
    ...
    real*8, parameter :: r0 = 0.0d0, r1 = 1.0d0, r3 = 3.0d0,           &
                         ...
                         r100 = 1.00d2
    real*8 :: acoff, zoutput
    
    external :: acoff, zoutput
    ...
    if (bflag_bubout .and. b_enable_output) then
      !igas = 999
      igsbub = lun_get()
    end if
    ...
    if (bubble_out .and. b_enable_output) then
...
!c  zone record for current time step
          if (b_output_binary) then
            ...
            write(igsbub,'(2a,1pe10.3,1x,a,3(a,i5),a)')                &
                 'zone t = "T_j, ',                                    &
...
!c calculate hydrodynamic pressure 
        ...
        tot_press = pres_atm+uvsnew(ivol)*rho_w*g_acc/pa_atm
...
#ifdef OPENMP
    if (any(sgsolve_failed .eqv. .true.)) then
...
#ifdef DEBUG  
    if (b_enable_output .and. bflag_dbg .and. mtime.eq.2 .and.         &
        ibub.eq.r1) then     
...
#ifdef DEBUG   
      if (b_enable_output .and. bflag_dbg) then
        write(idbg,'(i8,i8,i8)', ADVANCE='NO') mtime,ibub, ivol_max     
        write(idbg,'(50e15.7)') sanew_c(ivol_max), sanew(ivol_max),    &
...
!c update saturations using updatefactor of maximum update
    ...
    !$omp num_threads(numofthreads_global)                            &
    !$omp default(shared)                                             &
    !$omp private(ivol, izn, tid, delsa)                               
    !$omp do schedule(static)
#endif
    do ivol = 1,nngl
#ifdef OPENMP    
...
!c limit del Sa for very small timesteps
    ...
    if (b_enable_output .and. bflag_dbg) then
      write(idbg,'(i8,i8,i8)', ADVANCE='NO') mtime,ibub, ivol_max 
      write(idbg,'(50e15.7)') sanew_b(ivol_max), sanew_c(ivol_max),    &
...
!c calculate new component, species and gas concentrations based on new value of sa
    ...
    !$omp partial_pressure)                                           &
    !$omp firstprivate(delsa_max)                                      
    !$omp do schedule(static)
#endif
    do ivol = 1,nngl
#ifdef OPENMP    
...
!c  assign depth coordinate in terms of depth or elevation
        ...
        if (b_output_binary) then
          ...
          write(igsbub,'(6e15.7)',ADVANCE='NO')                        &
                xg(ivol),yg(ivol),zout,tot_press,sanew(ivol),          &
                r1-sanew(ivol)
          do ig = 1,ng
            i=gas_pair(ig)
            if(ig < ng) then
              write(igsbub,'(50e15.7)',ADVANCE='NO')                   &
                    aqueous_gas(ig),tot_gas(ig,ivol),cnew(i,ivol),     &
                    gnew(ig,ivol)
            else
              write(igsbub,'(50e15.7)')                                &
                    aqueous_gas(ig),tot_gas(ig,ivol),cnew(i,ivol),     &
                    gnew(ig,ivol)  
            end if
          end do 
