...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/opnmbfls.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
#include <finclude/petscsys.h>
      ...
      integer :: i, ianc, iaq, ic, im, imb, ig, isites, isb, l_sufx

      character*2 suffix
      character*2048 :: strbuffer
      
      integer*4 :: nvarsimvs, nvarsimrt, ilun, nlun
...
!c  mass balance - variably saturated flow
      ...
      if (mass_balance_vs) then
        ...
        !imvs_first  = 100
        imvs_first = lun_get()
        call lun_set(imvs_first+1)
        call lun_set(imvs_first+2)
...
!c  system mass
        ...
        if (rank == 0 .and. b_enable_output .and. b_output_trans_binary) then
          allocate(imvs_mpi(imvs_first:imvs_first+2))
          imvs_mpi = 0
          allocate(offset_imvs(imvs_first:imvs_first+2))
          offset_imvs = 0
          allocate(offset_imvs_ijk(imvs_first:imvs_first+2))
          offset_imvs_ijk = 0
        end if
...
#endif
            call binary_file_open(PETSC_COMM_SELF,             &
                         imvs_mpi(imvs), prefix(:l_prfx)//'_o.mve',    &
                         .true.)
          else
...
!c  write data to file information file
     ...
     &          'mol/day'                                              
          write(ifls,'(2a)')                                           &
     &          '8      mass influx by diffusion (gas phase)        ', &
     &          'mol/day'                                              
          write(ifls,'(2a)')                                           &
     &          '9      mass outflux by diffusion (gas phase)       ', &
     &          'mol/day'    
          write(ifls,'(2a)')                                           &
     &          '10     mass influx by advection (gas phase)        ', &
     &          'mol/day'                                              
          write(ifls,'(2a)')                                           &
     &          '11     mass outflux by advection (gas phase)       ', &
     &          'mol/day'  
          write(ifls,'(2a)')                                           &
     &          '12     change in storage (gas phase)               ', &
     &          'mol/day'                                              
          write(ifls,'(2a)')                                           &
     &          '13     mass loss - degassing                       ', &
     &          'mol/day'                                              
          write(ifls,'(2a)')                                           &
     &          '14     source/sink from sorbed phase               ', &
     &          'mol/day'                                              
          write(ifls,'(2a)')                                           &
     &          '15     total mass influx                           ', &
     &          'mol/elapsed time'                                     
          write(ifls,'(2a)')                                           &
     &          '16     total mass outflux                          ', &
     &          'mol/elapsed time'                                     
          write(ifls,'(2a)')                                           &
     &          '17     total change in storage                     ', &
     &          'mol/elapsed time'                                     
          write(ifls,'(2a)')                                           &
     &          '18     total source/sink intra-aqueous reactns     ', &
     &          'mol/elapsed time'                                     
          write(ifls,'(2a)')                                           &
     &          '19     total source/sink from mineral phase        ', &
     &          'mol/elapsed time'                                     
          write(ifls,'(2a)')                                           &
     &          '20     total source/sink from gas phase            ', &
     &          'mol/elapsed time'                                     
          write(ifls,'(2a)')                                           &
     &          '21     total mass influx by diffusion (gas phase)  ', &
     &          'mol/elapsed time'                                     
          write(ifls,'(2a)')                                           &
     &          '22     total mass outflux by diffusion (gas phase) ', &
     &          'mol/elapsed time'  
          write(ifls,'(2a)')                                           &
     &          '23     total mass influx by advection (gas phase)  ', &
     &          'mol/elapsed time'                                     
          write(ifls,'(2a)')                                           &
     &          '24     total mass outflux by advection (gas phase) ', &
     &          'mol/elapsed time'  
          write(ifls,'(2a)')                                           &
