...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/mbalvs.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
#include <finclude/petscsys.h>
      ...
      real*8 :: totvsstor, qroot_tot_act, vsstor, storvs, totinflux,   &
                totoutflux, totvsflux, fluxvs, fluxfs, qroot, rootwat, &
                absbalance, relbalance, culrelbalvs,rdummy1, rdummy2,  &
                fluxvs_dp

      external fluxvs, fluxvs_dp, fluxfs, msysvs, rootwat, storvs
...
!c  flux calculations
     ...
     &                                    cinfvs_a(i1))
                  ...
                  vsflux(icon) = - fluxvs_dp(upstream,hhead(ivol),     &
     &                                       hhead(jvol),              &
     &                                       uvsnew(ivol),             &
     &                                       uvsnew(jvol),             &
     &                                       relperm(ivol),            &
...
!c  compute contributions from root water uptake
    ...
    !$omp do schedule(static)
       ...
       do ivol = 1,nngl
          ...
          qroot_tot_act = qroot_tot_act + qroot
