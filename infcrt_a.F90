...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/infcrt_a.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
!c --------------------------------------------------------------------------
      ...
      subroutine infcrt_a (nvxgls, nvxgle, nvxgbl,                &
                         ...
                         type_averaging_De, xg, yg,               &
                         dual_permeability, sonew, oil_saturation)
...
#else
     use gen, only : rank, b_enable_output, tor_corr_a_mq, tor_corr_b_mq
#endif   
#ifdef PETSC
      ...
      external diffcoff, fluxfs, fluxvs, fluxvs_dp
      ...
      logical half_cells, fully_saturated, variably_saturated,    &
             ...
             dual_permeability,oil_saturation
      ...
      real*8  diffu, disx(nzn), disy(nzn), disz(nzn),             &
             ...
             tau_fac(nmax), marchies(nmax), xg(nmax), yg(nmax),   &
             sonew(nmax)

      real*8 diffcoff,fluxfs,fluxvs, fluxvs_dp
...
!c     local variables
      ...
      real*8 r0, rhalf, r1, r2, eps, tiny, pi
      parameter (eps = 1.0d-300, r0 = 0.0d0, rhalf = 0.5d0,       & 
                r1 = 1.0d0, r2 = 2.0d0,  tiny = 1.0d-10,          &
...
!cmj_dp  changed yg_i to yg(ibk) and yg_j to yg(id), indentation of else, endif was corrected too                
!cmj_dp                  if (yg(ibk).ne.yg(id)) then
                  if (dabs(yg(ibk)-yg(id)).gt.tiny) then 
                     cinfrt_va(iisav) = -fluxvs_dp(upstream,hhead(ibk),&
     &                                             hhead(id),          &
     &                                             uvsnew(ibk),        &
     &                                             uvsnew(id),         &
     &                                             relperm(ibk),       &
...
!cmj_dp  added "if (yg_i.ne.yg_j) then" to check for cases when dual_permeability
!cmj_dp  is ok but running in x/z direction-Mehrnoush-19/04/2013-NOT SURE-->MAY NEED TO CHANGE BACK!
!cmj_dp I don't need this cause fluxvs_dp can handle the situation inseide the function.
                       ...
                       if (.not.dual_permeability) then 
                         ...
                         if (dabs(yg(ibk)-yg(id)).gt.tiny) then 
                           dflux = - fluxvs_dp(upstream,hhead(ibk), &
                                             hhead(id),             &
                                             uvsnew(ibk),           &
                                             uvsnew(id),            &
                                             relperm(ibk),          &
