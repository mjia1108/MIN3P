...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/velocity_a.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
#include <finclude/petscsys.h>
    ...
    real*8 :: distance, xg_out, yg_out, zg_out
    real*8 :: fluxd, fluxv_vl, fluxd_dp, fluxv_vl_dp

    external cliqdisp, totconc, elecmigration
    external fluxd, fluxv_vl, fluxd_dp, fluxv_vl_dp
...
!c  local variables
    ...
    real*8, parameter :: r1 = 1.0d0, rverysmall = 1.d-30
     ...
     &       aflux_dif(n),aflux_mig(n),aflux_tot(n),diff(n,3), dd(3,12),&
     &       vel(3),dist(3,12),d(3,nmax),areax(3,12)

      integer ivol, ivol2, ivz, ivy, ivx, i1, jvol, i1sav, idim, npair(3),     &
     &        cvpair(3,12,2), ipair, idim2, idim3,             &
...
!     &           'Average interfacial aqueous phase velocities, T = ',
...
!     &        '7        p_t_o_t                         ','atm'
      ...
      if (b_output_binary) then
        tec_variables = [character(len=72) ::                          &
...
!c  compute aqueous phase flux
     ...
     &                                ivol,jvol,         &
     &                                cinfrt_va(i1),ic)     
            aflux_dif(ic) = - fluxd(totviscnew(ic,ivol),      &     !diffusive term
     &                             totviscnew(ic,jvol),     &
     &                             cinfrt_mcd(i1))
            aflux_mig(ic) = - fluxd(electromignew(ic,ivol),r0,    &
...
!c ---------------- fick's law, business as usual -------------------------     
          ...
          if (.not.dual_permeability) then
            ...
            if ((yg(ivol) > mat_frac_face_y .and.              &
              ...
              aflux_adv(ic) = fluxv_vl_dp(totcnew(ic,ivol),    &    !advective term
                                     totcnew(ic,jvol),         &
                                     ivol,jvol,                &
                                     cinfrt_va(i1),ic)
              aflux_dif(ic) = - fluxd_dp(totcnew(ic,ivol),     &    !dispersive term
                                     totcnew(ic,jvol),         &
                                     cinfrt_da(i1),            &
                                     cinfvs_a(i1),             &
...
!c           mass fluxes
            ...
            do ic=1,n
              ...
              if (b_output_binary) then
                ...
                write(igsa2,'(18e15.6e3)') xg_out,yg_out,zg_out,       &
                                     (vela_adv(ic,idim),idim=1,3),     &
                                     (vela_dif(ic,idim),idim=1,3),     &
                                     (vela_mig(ic,idim),idim=1,3),     &
                                     (vela_tot(ic,idim),idim=1,3)
