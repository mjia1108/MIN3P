...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/dratemin_salinity.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
!c a and b is determined by providing the x values when f=0 and 1 from the input file.
          ...
          if ((sar .lt. 0.0 .or. sar .gt. 1.0) .or.  &
            ...
            if (rank == 0 .and. b_enable_output) then
                ...
                write(ilog,'(a,e15.8)') 'The salinity = ',salinity
                write(ilog,'(a,e15.8)') 'The salinityinc = ',salinityinc
                write(ilog,'(a,e15.8)') 'The coefficient sar = ',sar
                write(ilog,'(a,e15.8)') 'The coefficient sarinc = ',sarinc
                write(ilog,*) 'It should be between 0.0 to less or equal 1.0.'
                do ic = 1, nc-1
                    write(ilog,'(3a,e15.8)') 'The totc(', namec(ic),') = ',totc(ic)
                    write(ilog,'(3a,e15.8)') 'The totcinc(', namec(ic),') = ',totcinc(ic,tid)
                end do
      ...
      if (rank == 0 .and. b_enable_output .and. info_debug.gt.0 .and. (ivol .eq. 2 &
        ...
        write(ilog,'(a,e15.8)') 'The salinity = ',salinity
        write(ilog,'(a,e15.8)') 'The Cl- concentration = ',totc(inamec_inhib(im))
        write(ilog,'(a,e15.8)') 'The coefficient sar = ',sar
        write(ilog,'(a,e15.8)') 'The coefficient ratem = ',ratem
      end if
