...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/initprob.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
#include <finclude/petscsys.h>
     ...
     &         initbcrt,indextec,inittemp,initweed,initdualperm,      &
...
!c  control parameters for dual permeability
...
!cprovi---------------------------------------------------------------    
      ...
      if (i_solver_type_react == 0 .or. i_solver_type_flow == 0) then
        ...
        if(b_min3p_input_param_first) then
            ...
            if(heat_transport) then
                lis_abstol_flow = restolglob
                lis_maxiter_flow = msolvitglob
                write(strbuffer,*) levelglob
            else
                lis_abstol_flow = restol_vs
                lis_maxiter_flow = msolvit_vs
            ...
            write(strbuffer,*) lis_maxiter_flow               
            lis_options_flow = trim(lis_options_flow)//      &
                               " -maxiter "//trim(strbuffer)
            
            
            lis_abstol_react = restol_rt
            lis_maxiter_react = msolvit_rt
            ...
            write(strbuffer,*) lis_maxiter_react               
            lis_options_react = trim(lis_options_react)//    &
                               " -maxiter "//trim(strbuffer)
