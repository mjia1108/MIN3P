...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/timeloop.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
!c ----------------------------------------------------------------------
      ...
      integer :: tid, i, i1, igb, iprint, igb_step, info_debug, ivol,  &
                 igb_step_ijface,ngb_tstep_ijface,                     &
                 ipest, istatus, ierrcode, ngb_tstep, iunit, itid
      real*8 :: cputime, cpuint_old, porosity_flow, bubm
...
#include <finclude/petscsys.h>
      ...
      external mbalvs, mbaldd, nexttime, outputvs,                     &
               ...
               tranunit,updtbcvs, updtbcdd, updtsoil, velodd,          &
               tranbcrt, initfaceflux, tprfvs_faceflux,                &
               tprfrtlc_faceflux,tprfrtlc_g_faceflux
     ...
     &           r365 = 3.65d2, r3 = 3.0d0, r10 = 1.0d-10
      ...
      !Local variables
      integer :: ibub_tol, ibubflow_tol, ic, izn 
...
!c option look for file pest.stp, which will cause simulation to abort
      ...
      do while (time.lt.tfinal-tiny_time)       !start time loop
...
!c  every ngb_step_faceflux time step and after completion of solution
        ...
        if (gb_output_faceflux .and. b_enable_output) then         !**Note: Output is not parallelized yet, DSU
          ...
          if (igb_step_ijface.eq.ngb_step_ijface.or.time.gt.tfinal-tiny_time) then
            ...
            if (reactive_transport) then                
              do igb = 1,ngb_ijface
                call tprfrtlc_faceflux(ngb_vol_ijface(1,igb),          &
                                       ...
                                       ngb_tstep_ijface+1)  
                if (ng > 0) then
                  call tprfrtlc_g_faceflux(ngb_vol_ijface(1,igb),          &
                                           ngb_vol_ijface(2,igb),igb,      &
                                           ngb_tstep_ijface+1)
                end if
              end do
...
!cprovi----------------------------------------------------
!cprovi update soil specific parameters
!cprovi----------------------------------------------------
        if (root_uptake) then
