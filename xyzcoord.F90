...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/xyzcoord.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
!c  calculate coordinates
      do ivx = 1,nvxgbl            !number of control volumes in x
        ...
        if(info_debug > 0) then
            write(idbg,'(a,1x,i6,1x,a,1x,e13.6)') "ivx",ivx,           &
                  "xglat",xglat(ivx)
        end if
#endif
...
#ifdef DEBUG
        if(info_debug > 0) then
            write(idbg,'(a,1x,i6,1x,a,1x,e13.6)') "ivy",ivy,           &
                  "yglat",yglat(ivy)
        end if
#endif
...
#ifdef DEBUG
        if(info_debug > 0) then
            write(idbg,'(a,1x,i6,1x,a,1x,e13.6)') "ivz",ivz,           &
                  "zglat",zglat(ivz)
        end if
#endif
...
#ifdef OPENMP
#ifdef PARALLEL_SECTION
    !$omp parallel private(ivol, ivx, ivy, ivz)
    !$omp sections
#endif
#endif  
