...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/readmin_new.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
!c           diffm(ndr*nm)      = diffusion coefficient of primary    * +
...
!c local:    real*8:
!c           -------
!c           ordt(10)           = array for temporary storage of
!c                                reaction orders
!c           r0                 = constant
!c           r1                 = constant
!c           r10                = constant
!c           r86400             = constant
!c           value(10)          = array for temporary storage
!c           value2(10)         = array for temporary storage
!c           xnumt(10)          = array for temporary storage of
!c                                stoichiometric coefficients of
...
!c ----------------------------------------------------------------------
      ...
      subroutine readmin_new(imdbs,ilog,idbg,icnv)
      ...
      integer :: i, i1, i2, ic, icount, icur, idc, idcx, idt, im, im1, &
                 ...
                 ndsta,nifr, ntsct, nprsc, nprs, idummy
      
      real*8 :: dummy
      real*8 :: xnumt(10),ordt(10),value(10),value2(10), ntsct2(10),   &
                alphat(10,10)
      integer :: iinum(10)
                ...
                backspace(imdbs)
                  ...
                  if(ordn(ireac) /= 1.0d0 .and. b_ratelaw_exponent_n .eqv. .false.) then
                      if (rank == 0 .and. b_enable_output) then
                        write(*,*) 'WARNING: supplied exponent n will be ',     &
         &                           'ignored and is set to 1'
                        write(ilog,*) 'WARNING: supplied exponent n will be ',  &
         &                           'ignored and is set to 1'
                      end if
                  end if
...
!c  convert diffusion coefficient to internal time units
                    ...
                    diffm(ireac) = diffm(ireac)*r86400
            ...
            searching = .true.
              ...
              do while (searching.and..not.found)
                ...
                backspace(imdbs)
                  ...
                  do idcx = 1,ndcx
                      read(imdbs,*,err=999,end=998) namet(idcx),      &
     &                                              ordt(idcx)
                  end do
      ...
      if (info_debug.gt.0) then
        ...
        do im=1,nm
          ...
          if (.not.far_from_equil(im)) then
            write(idbg,'(a,e10.3)') 'eqms  ',-dlog10(eqms(im))
          end if
          write(idbg,'(a,f10.3)') 'gfwm  ',gfwm(im)
          write(idbg,'(a,f10.3)') 'densm ',densm(im)
          write(idbg,'(a,f10.3)') 'dhcm  ',dhcm(im)
          write(idbg,'(a,f10.3)') 'dgcm  ',dgcm(im)
...
!c  echo check for stoichiometric coefficients
          ...
          do i = istart, iend
            icur = jam(i)
            write(idbg,'(i5,2x,a72,a,i5,a,e10.3)') i,namec(icur),     &
...
!c  echo check rate constant and affinity term
          ...
          iend = iamd(im+1)-1
          do i = istart, iend
            write(idbg,'(i5,4(a,e10.3))') i,                          &
     &                                   ' ordm ',ordm(i),            &
     &                                   ' ordn ',ordn(i),            &
     &                                   ' rateds',                   &
     &                                   dlog10(rateds(i)/r86400)
...
!c  echo check - fractional order terms - total concentrations
            ...
            do i2 = istart2,iend2                                      
              icur = jamdt(i2)                                         
              write(idbg,'(i5,2x,a72,a,i5,a,e10.3)') i2,namec(icur),  &
     &                                          ' jamdt ',jamdt(i2),  &
     &                                          ' orddt ',orddt(i2)
            end do
...
!c  echo check - fractional order terms - aqueous complexes
            ...
            do i2 = istart2,iend2                                      
              icur = jamdcx(i2)                                        
              write(idbg,'(i5,2x,a72,a,i5,a,e10.3)') i2,namex(icur),  &
     &                                          ' jamdcx ',jamdcx(i2),&
     &                                          ' orddcx ',orddcx(i2)  
            end do
...
!c  echo check - hyperbolic terms - T^a
            ...
            do i2 = istart2,istop2                                     
              icur = jamdm(i2)                                         
              write(idbg,'(i5,2x,a12,a,i5,2(a,e10.3))') i2,&
     &                            namec(icur),  ' jamdm ',jamdm(i2),  &
     &                                          ' fmdm ',fmdm(i2),    &
     &                                          ' ordmdm ',ordmdm(i2)  
            end do
...
!c  echo check - hyperbolic terms - C^c
            ...
            do i2 = istart2,istop2
              icur = jamhc(i2)
              write(idbg,'(i5,2x,a72,a,i5,2(a,e10.3))') i2,namec(icur), &
     &                                          ' jamhc ',jamhc(i2),    &
     &                                          ' fmhc ',fmhc(i2),      &
     &                                          ' ordmhc ',ordmhc(i2)    
            end do
