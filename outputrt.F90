...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/outputrt.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
#include <finclude/petscsys.h>
      ...
      real*8 :: alk_carb, alk_noncarb, alk_tot, alk_tot_mg,            &
                ...
                frac_mol, por_out, gammatemp, tottemp
      
      integer :: i, ianc, iaq, ic, ig, im, imx, iout_high, ir, ireac,  &
                 istart, istop, isites, isb, ivol, ix, jg, l_sufx,     &
                 nxout, ivol_l, nvarsgsc, nvarsgsm, nvarsgsi,          &
                 nvarsgsb, nvarsgsd, nvarsgsv, nvarsgsis, ierrcode
      ...
      external alkcalc,comptotc,phpe,                                 &
               rateredx,satindex,sorbspc,totcona,totconc,             &
               zoutput, molconc, rategas, rategasd, rateint,          &
               rateint_new, ratemin, ratemin_new, tcorr, updtsvap,    &
               modrate
...
!c  version information
     ...
     &          '3        z                               ','m'        
          do ir = 1,nr                                                 
            if (ir.lt.7) then                                          
              write(ifls,'(i1,8x,a18,2x,a)') ir+3,namer(ir),          &
     &                                     'moles/l h2o/day'           
            else                                                       
              write(ifls,'(i2,7x,a18,2x,a)') ir+3,namer(ir),          &
     &                                     'moles/l h2o/day'
            end if
          end do
...
!c  rates of intra-aqueous kinetic reactions
     ...
     &            '3        z                               ','m'       
            do iaq = 1,naq                                              
              if (iaq.lt.7) then                                        
                write(ifls,'(i1,8x,a18,2x,a)') iaq+3,nameaq(iaq),      &
     &                                       'moles/l h2o/day'          
              else                                                      
                write(ifls,'(i2,7x,a18,2x,a)') iaq+3,nameaq(iaq),      &
     &                                       'moles/l h2o/day'
              end if
            end do
...
!c  partial gas pressures
     ...
     &                               'condition',('-',i=1,72)           
              ...
              write(ifls,'(/2a,1pe12.4,1x,a/72a)')                     &
     ...
     &           '3+ng+1   total gas pressure              ','atm'
            ...
            if ((.not.initial_condition).and.flux_out) then
              call velocity_g_ex (l_sufx, suffix, nngl, njavs, cinfrad,   &
                                  radial_coord)
            end if            
...
!c gas velocities, advection
     ...
     &              '3        z                               ','m'
              do ig = 1,ng
                if (ig.lt.7) then
                  write(ifls,'(i1,8x,a,20x,a)') ig+3,nameg(ig),'[-]'
                else
                  write(ifls,'(i2,7x,a,20x,a)') ig+3,nameg(ig),'[-]'
                end if
              end do
...
!c  reaction rates of minerals
     ...
     &            '3        z                               ','m'         
            ...
            do ireac = istart,istop                                       
              if (ireac.lt.7) then                                        
                write(ifls,'(i1,8x,a28,2x,a)') ireac+3,namemp(ireac),   &
     &                                        'moles/l bulk/day'          
              else                                                        
                write(ifls,'(i2,7x,a28,2x,a)') ireac+3,namemp(ireac),   &
     &                                        'moles/l bulk/day'
              end if
            end do
...
!c  zone record for current time step
          if (b_output_binary) then
            ...
            write(igst,'(2a,1pe10.3,1x,a,3(a,i5),a)')                  &
                 'zone t = "T_j, ',                                    &
...
! prc ----------------------------------------------------------------
          if (multi_diff) then
            if (b_output_binary) then  
              ...
              write(icbt,'(2a,1pe10.3,1x,a,3(a,i5),a)')                &
                   'zone t = "T_j, ',                                  &
                   'T = ',time_io,time_unit(:l_time_unit),             &
                   '", i =',itec,', j =',jtec,', k =',ktec,',  f=point'
            end if            
          ...
          if(flux_out)then
            if (b_output_binary) then
              ...
              if (b_output_multizone) then
               ...
               write(igmf,'(2a,1pe10.3,1x,a,3(a,i5),a)')               &
               'zone t = "T_j, ',                                      &
               'T = ',time_io,time_unit(:l_time_unit),                 &
               '", i =',itec,', j =',jtec,', k =',ktec,',  f=point'
            end if
...
!c  assign depth coordinate in terms of depth or elevation
        ...
        if (b_output_binary) then
          ...
          if (iout_high .eq. 1) then
              write(igst,'(3e15.6e3,50e23.15e3)')                      &
                                      xg(ivol),yg(ivol),zout,          &
                                      (totcnew(ic,ivol),ic=1,nc-1)
          else                                                         
              write(igst,'(50e15.6e3)') xg(ivol),yg(ivol),zout,        &
                                      (totcnew(ic,ivol),ic=1,nc-1)
          end if
        end if
! prc ----------------------------------------------------------------
...
! prc ----------------------------------------------------------------
        ...
        if (multi_diff) then
            do ic =1,nc-1
               zc(ivol) = zc(ivol) + totcnew(ic,ivol)*chargec(ic)
               zcabs(ivol)=zcabs(ivol)+abs(totcnew(ic,ivol)*chargec(ic))
            end do
...
! prc ----------------------------------------------------------------
            if (b_output_binary) then
              ...
              realbuffer2(ivol_l*(4+nc)-1) = 100*zc(ivol)/zcabs(ivol)
              realbuffer2(ivol_l*(4+nc)) = zc(ivol)
            else
              write(icbt,'(50e15.6e3)') xg(ivol),yg(ivol),zout,        &
                   (totcnew(ic,ivol),ic=1,nc-1),                       &
                    100*zc(ivol)/zcabs(ivol),zc(ivol)
            end if
      end if
      ...
      if(flux_out)then
        ...
        if (b_output_binary) then
          ...
          write(igmf,'(50e15.6e3)') xg(ivol),yg(ivol),zout,            &
...
!c  title and variables
          if (b_output_binary) then
            ...
            write(igsc,'(600a)') 'variables = "x", "y", "z"',          &
                                (',"',namec(ic)(:l_namec(ic)),         &
                                '"',ic=1,nc-1),                        &
                                (',"',namex(ix)(:l_namex(ix)),         &
                                '"',ix=1,nxout)
...
!c  zone record for current time step
            if (b_output_binary) then
              ...
              write(igsc,'(a,1pe10.3,1x,a,3(a,i5),a)')                 &
                   'zone t = "C_j, X_i, T = ',                         &
...
!c  zone record for current time step
            if (b_output_binary) then
              ...
              write(igsm,'(a,1pe10.3,1x,a,3(a,i5),a)')                 &
...
!c  zone record for current time step
            if (b_output_binary) then
              ...
              write(igsi,'(2a,1pe10.3,1x,a,3(a,i5),a)')                &
...
!c  zone record for current time step
              if (b_output_binary) then
                ...
                write(igsg,'(a,1pe10.3,1x,a,3(a,i5),a)')               &
                     'zone t = "G_i, T = ',                            &
                     time_io,time_unit(:l_time_unit),'", i =',         &
                     itec,', j =',jtec,', k =',ktec,',  f=point'
              end if
              ...
              if (gas_advection .or. dgm .or. maxwell) then
                if (b_output_binary) then
                  ...
                  write(igs2,'(a,1pe10.3,1x,a,3(a,i5),a)')             &
                       'zone t = "G_i, T = ',                          &
                       time_io,time_unit(:l_time_unit),'", i =',       &
                       itec,', j =',jtec,', k =',ktec,',  f=point'
                end if
...
!c  zone record for current time step
                if (b_output_binary) then
                  ...
                  write(igsgr,'(a,1pe10.3,1x,a,3(a,i5),a)')            &
                       'zone t = "R_i^g, T = ',                        &
                       time_io,time_unit(:l_time_unit),'", i =',       &
                       itec,', j =',jtec,', k =',ktec,',  f=point'
                end if
...
!c  zone record for current time step
              if (b_output_binary) then
                ...
                write(igsb,'(a,1pe10.3,1x,a,3(a,i5),a)')               &
                     'zone t = "S_i, T = ',                            &
                     time_io,time_unit(:l_time_unit),'", i =',         &
                     itec,', j =',jtec,', k =',ktec,',  f=point'
              end if
...
!c  zone record for current time step
              if (b_output_binary) then
                ...
                write(igss,'(a,1pe10.3,1x,a,3(a,i5),a)')               &
                     'zone t = "SI, T = ',                             &
                     time_io,time_unit(:l_time_unit),'", i =',         &
                     itec,', j =',jtec,', k =',ktec,',  f=point'
              end if
...
!c  zone record for current time step
              if (b_output_binary) then
                ...
                write(igsd,'(2a,1pe10.3,1x,a,3(a,i5),a)')              &
                    'zone t = "Diss.-prec. rates, ',                   &
                    'T = ',time_io,time_unit(:l_time_unit),            &
                    '", i =',itec,', j =',jtec,', k =',ktec,',  f=point'
              end if
...
!c  title and variables 
            ...
            if (b_output_binary) then
              ...
              tec_variables(5+nm) = "hydraulic conductivity"
...
!c  zone record for current time step
              if (b_output_binary) then
                ...
                write(igsv,'(a,1pe10.3,1x,a,3(a,i5),a)')               &
                    'zone t = "phi_i, T = ',                           &
                    time_io,time_unit(:l_time_unit),                   &
                    '", i =',itec,', j =',jtec,', k =',ktec,',  f=point'
              end if
...
!c  zone record for current time step
              if (b_output_binary) then
                ...
                write(igsx,'(2a,1pe10.3,1x,a,3(a,i5),a)')              &
                    'zone t = "SI, ',                                  &
                    'T = ',time_io,time_unit(:l_time_unit),            &
                    '", i =',itec,', j =',jtec,', k =',ktec,',  f=point'
              end if
...
!c  zone record for current time step
              if (b_output_binary) then
                ...
                write(igsis,'(2a,1pe10.3,1x,a,3(a,i5),a)')             &
                    'zone t = "isotope data, ',                        &
                    'T = ',time_io,time_unit(:l_time_unit),            &
                    '", i =',itec,', j =',jtec,', k =',ktec,',  f=point'
              end if
...
!c  zone record for current time step
              if (b_output_binary) then
                ...
                write(igsw,'(a,1pe10.3,1x,a,3(a,i5),a)')               &
                     'zone t = "G_i, T = ',                            &
                     time_io,time_unit(:l_time_unit),'", i =',         &
                     itec,', j =',jtec,', k =',ktec,',  f=point'
              end if
...
!c  zone record for current time step
              ...
              write(igsg,'(2a,1pe10.3,1x,a,3(a,i5),a)')               &
     &             '# zone t = "G_i, ',                               &
     &             'T = ', time_io,time_unit(:l_time_unit),           &
     &             '", i =',itec,', j =',jtec,', k =',ktec,',  f=point'
...
!c  zone record for current time step
                ...
                write(igsgr,'(2a,1pe10.3,1x,a,3(a,i5),a)')            &
     &               '# zone t = "R_i^g, ',                           &
     &               'T = ', time_io,time_unit(:l_time_unit),         &
...
!c  --------------------------------------------------------------------- 
        if (b_output_binary .and. extended_output) then          
          offset_igsc_temp = offset_igsc          
          ...
          allocate(realbuffer(nn*nvarsgsc))
          ...
          if (ph_output .or. pe_output) then
            ...
            call tecplot_binary_write_section(Petsc_Comm_World,igsm,   &
                         ...
                         b_output_mpiio_single,.false.,                &
                         b_output_multizone) 
            allocate(realbuffer2(nn*nvarsgsm))
            realbuffer2 = 0.0d0
          end if
          ...
          if ((naq.eq.0 .and. nr.gt.0 .and. .not. redox_equil .and.    &
            ...
            call tecplot_binary_write_section(Petsc_Comm_World,igsi,   &
                         ...
                         b_output_mpiio_single,.false.,                &
                         b_output_multizone) 
            allocate(realbuffer3(nn*nvarsgsi))
            realbuffer3 = 0.0d0
          end if
          ...
          if (nsb_ion.gt.0 .or. nsb_surf.gt.0 .or.                     &
            ...
            call tecplot_binary_write_section(Petsc_Comm_World,igsb,   &
                         ...
                         b_output_mpiio_single,.false.,                &
                         b_output_multizone) 
            allocate(realbuffer7(nn*nvarsgsb))
            realbuffer7 = 0.0d0
          end if
          ...
          if (nm.gt.0) then
            ...
            call tecplot_binary_write_section(Petsc_Comm_World,igsd,   &
                         ...
                         b_output_mpiio_single,.false.,                &
                         b_output_multizone) 
            allocate(realbuffer9(nn*nvarsgsd))
            realbuffer9 = 0.0d0
            ...
            call tecplot_binary_write_section(Petsc_Comm_World,igsv,   &
                         ...
                         b_output_mpiio_single,.false.,                &
                         b_output_multizone) 
            allocate(realbuffer10(nn*nvarsgsv))
            realbuffer10 = 0.0d0
          end if
          ...
          if (nmx .gt. 0) then
            ...
            call tecplot_binary_write_section(Petsc_Comm_World,        &
                         ...
                         b_output_mpiio_single,.false.,                &
                         b_output_multizone) 
            allocate(realbuffer11(nn*(3+nmx)))
            realbuffer11 = 0.0d0
          end if
          ...
          if (iso_output) then
            ...
            call tecplot_binary_write_section(Petsc_Comm_World,        &
                         ...
                         b_output_mpiio_single,.false.,                &
                         b_output_multizone) 
            allocate(realbuffer12(nn*nvarsgsis))
            realbuffer12 = 0.0d0
          end if
...
!c  free species and aqueous complex concentrations
          if (extended_output) then
            if (b_output_binary) then
              ...
              write(igsc,'(1000e15.6e3)') xg(ivol),yg(ivol),zout,      &
     ...
     &                   iax,jax,nc,nx,namec,namex)                    
          ...
          elseif ((.not.ph_output).and.(pe_output)) then
            if (b_output_binary) then
              ...
              write(igsm,'(14e15.6e3)') xg(ivol),yg(ivol),zout,pe,eh,  &
                                   sionnew(ivol),alk_carb,alk_noncarb, &
                                   alk_tot,alk_carb_mg,alk_noncarb_mg, &
                                   alk_tot_mg,tkel(ivol)-tconv,zbal
            end if
          elseif ((ph_output).and.(.not.pe_output)) then
            if (b_output_binary) then  
              ...
              write(igsm,'(13e15.6e3)') xg(ivol),yg(ivol),zout,ph,     &
                                   sionnew(ivol),alk_carb,alk_noncarb, &
                                   alk_tot,alk_carb_mg,alk_noncarb_mg, &
                                   alk_tot_mg,tkel(ivol)-tconv,zbal
            end if
...
!c  recompute oxidation-reduction rates
     ...
     &                      totcnew(1,ivol),ir,tid)                        
              ...
              if (b_output_binary) then
                ...
                write(igsi,'(50e15.6e3)') xg(ivol),yg(ivol),zout,      &
...
!c  partial gas pressures and total gas pressure
            ...
            if (b_output_binary) then
              ...
              write(igsg,'(50e15.6e3)') xg(ivol),yg(ivol),zout,        &
                                 (rgasatm*tkel(ivol)*gnew(ig,ivol),    &
                                  ig=1,ng),pres_tot
            end if
            ...
            if (gas_advection .or. dgm .or. maxwell) then
              if (b_output_binary) then
                ...
                write(igs2,'(8e15.6e3)') xg(ivol),yg(ivol),zout,       &
...
!c  components due to degassing [mol L^-1 h2o s^-1]
            ...
            if (gas_removal) then
              ...
              if (b_output_binary) then
                ...
                write(igsgr,'(50e15.6e3)') xg(ivol),yg(ivol),zout,     &
...
!c  non-competitive sorption only
            if (b_output_binary) then
              ...
              write(igsb,'(50e15.6e3)') xg(ivol),yg(ivol),zout,        &
                                      (totan(ianc,tid),                &
                                      ianc=1,nanc)
            end if            
          ...
          elseif (.not.noncompetitive_sorption) then
              ...
              if (trim(output_unit_sb_surf) .eq. 'mol/L H2O') then
                ...
                if (b_output_binary) then
                  ...
                  do isb=1,nsb_surf
                    realbuffer7((ivol_l-1)*nvarsgsb+3+nsites+nsb_ion+  &
                              isb) = csb_surf(isb,tid)*sanew(ivol)
                  end do  
                else
                  write(igsb,'(50e15.6e3)')                            &
                        xg(ivol),yg(ivol),zout,                        &
                        ...
                        isites=1,nsites),                              &
                        (csb_ion(isb,tid),isb=1,nsb_ion),              &
                        (csb_surf(isb,tid)*sanew(ivol),isb=1,nsb_surf) 
                end if
              end if
...
!c  competitive and non-competitive sorption
              ...
              if (trim(output_unit_sb_surf) .eq. 'mol/L H2O') then
                ...
                if (b_output_binary) then
                  ...
                  do isb=1,nsb_surf
                    realbuffer7((ivol_l-1)*nvarsgsb+3+nanc+nsites+     &
                            nsb_ion+isb) = csb_surf(isb,tid)*sanew(ivol)
                  end do 
                else
                  write(igsb,'(50e15.6e3)')                            &
                        xg(ivol),yg(ivol),zout,                        &
...
!c  skip calculation for far-from-equilibrium reactions
            ...
            if (b_output_binary) then
              ...
              write(igss,'(50e15.6e3)') xg(ivol),yg(ivol),zout,        &
                                      (satm_log(im),im=1,nm)
            end if
...
!c  assembly
              ...
              call modrate(ratemdp(im,ivol),cmnew(im,ivol),delt,im,tid)
...
!cdsu  Assign mineral rate for frozen water
            ...
            if (b_output_binary) then
              ...
              write(igsd,'(50e15.6e3)') xg(ivol),yg(ivol),zout,      &
            ...
            if (update_porosity .and. update_permeability) then
              if (b_output_binary) then
                ...
                realbuffer10((ivol_l-1)*nvarsgsv+5+nm) =               &
                             condzz(nzn)*perm_fac(ivol)/sec_per_days  
              else
                write(igsv,'(50e15.6e3)') xg(ivol),yg(ivol),zout,      &
                                        (phi_out(im),im=1,nm),         &
                                         pornew(ivol),                 &
                                condzz(nzn)*perm_fac(ivol)/sec_per_days  !MX temp
              end if                                                         
              ...
              if (b_output_binary) then
                ...
                realbuffer10((ivol_l-1)*nvarsgsv+4+nm) = por_out 
              else
                write(igsv,'(50e15.6e3)') xg(ivol),yg(ivol),zout,      &
                                        (phi_out(im),im=1,nm),pornew(ivol)
              end if
            end if
          end if    !(nm.gt.0)
...
!c  saturation indices for excluded minerals
          ...
          if (nmx.gt.0) then
            ...
            if (b_output_binary) then
              ...
              write(igsx,'(50e15.6e3)') xg(ivol),yg(ivol),zout,        &
...
!c isotope data          
          if (iso_output) then
            ...
            if (b_output_binary) then
              ...
              write(igsis,'(50e15.6e3)') xg(ivol),yg(ivol),zout,       &
...
!c  ---------------------------------------------------------------------
          ...
          if (chemical_water) then
            ...
            if (b_output_binary) then
		      ...
		      write(igsw,'(4e15.6e3)') xg(ivol),yg(ivol),zout,qwater(ivol)
