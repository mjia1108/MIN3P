...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/outputvs.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
#include <finclude/petscsys.h>
      ...
      integer :: i, ivol, l_sufx, ivol_l, ierrcode
      
      real*8 :: qroot, rootwat, zout, zoutput, theta_a, theta_g, phead
        ...
        if (fully_saturated) then
          ...
          if (b_output_binary) then
            ...
            call tecplot_binary_write_variable(Petsc_Comm_World, igsp, &
                         7, tec_variables(1:7), offset_igsp,           &
                         b_output_mpiio_single,.false.)   
          else
        ...
        elseif (variably_saturated) then
          ...
          if (b_output_binary) then
            ...
            tec_variables(1:10) = [character(len=72) :: "x", "y", "z", &
                                  "h_w", "p_w","s_w",                  &
                                  "theta_a", 's_a', 'theta_g','q_root']
            
            call tecplot_binary_write_variable(Petsc_Comm_World, igsp, &
                         10, tec_variables(1:10),offset_igsp,          &
                         b_output_mpiio_single,.false.)
...
!c  ---------------------------------------------------------------------
      if (b_output_binary) then
        ...
        offset_igsp_temp = offset_igsp
        if (fully_saturated) then            
          call tecplot_binary_write_section(Petsc_Comm_World,igsp,7,   &
                       nn_offset,offset_igsp,b_output_mpiio_single,    &
                       .false.,b_output_multizone)
        else if (variably_saturated) then
          call tecplot_binary_write_section(Petsc_Comm_World,igsp,10,  &
                       nn_offset,offset_igsp,b_output_mpiio_single,    &
                       .false.,b_output_multizone) 
        end if
...
!c  assign depth coordinate in terms of depth or elevation
        ...
        if (fully_saturated) then
          ...
          if (b_output_binary) then
            realbuffer((ivol_l-1)*7+1:ivol_l*7)=(/xg(ivol),yg(ivol),   &
                       zout,hhead(ivol),phead,sanew(ivol),theta_a /)
            
          else
          
            write(igsp,'(7e15.6e3)') xg(ivol),yg(ivol),zout,hhead(ivol), &
        ...
        elseif (variably_saturated) then
          ...
          if (b_output_binary) then
            realbuffer((ivol_l-1)*10+1:ivol_l*10)=(/xg(ivol),        &
                   ...
                   sanew(ivol),theta_a,sgnew(ivol), theta_g, qroot /)
          else
            write(igsp,'(10e15.6e3)') xg(ivol),yg(ivol),zout,hhead(ivol),&
     ...
     &                                sgnew(ivol), theta_g, qroot   
      ...
      if (.not.initial_condition) then
        ...
        call velocity(nvxgl, nvygl, nvzgl, iavs, javs, cinfvs_a, dimcv,&
                      ...
                      njavs, nngl, nn, half_cells,                     &
                      dual_permeability, cinfrad,                      &
