...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/jacbrt.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
!c ----------------------------------------------------------------------
      ...
      integer :: idiag, i1, i2, ibl, ibvs, icol, ierr, ig, info_debug, &
                 ...
                 irow, iaq, jvol, jbl, ldiag, i, j, jtemp

      
      real*8 :: acoff, area_ivol, ddbdflux, bdryflux, bdyinfrt_da,     &
                bdyinfrt_dg, dgflux, dfluxvg, diff_eff, diffcoff,      &
                drtinc, fluxd, fluxd_dp, fluxvg, gasm, gasp_m, gasd_m, &
                gasv, gasp, gasdiff2, diffcoff_g, fluxd_g_dp

      
      integer :: tid, ivol_track, ivol_gbl

      external acoff, bdryflux, comptotc, ddbdflux, diffcoff,dtotcong, &
               fluxd, secspec, totconc, gasm, gasd_m, gasp_m, gasv,    &
               giups_brt, wgprop, wgpropd, gasp, atotconc, dtotconc,   &
               gasconc, tcorr, gcreact, jacbevap, fluxd_g_dp
!cprovi-------------------------------------------------------------------
...
!cprovi-------------------------------------------------------------------
      real*8, parameter :: r0 = 0.0d0, r1 = 1.0d0, rsmall=1.0d-1,      &
                           rtiny = 1.0d-10, rverysmall=1.0d-30,        &
                           gsatmin=0.0d0
...
!c    assign gas advection spatial weighting
    ...
    !$omp jvol, ivol_gbl, gsatijbrt, gporijbrt, tauijbrt,             &
    !$omp mdens_g_ivol, mdens_g_brt,                                  &     !Gas transport variables
...
!c diffusion expressed in terms of concentration gradients
     ...
     &                               cinfrt)                               !bdyinfrt_dg
                   ...
                   !totgflux(ic) = totgflux(ic)                              & !diffusion flux
                   !             + fluxd_g_dp(totgnew(ic,ivol),              & 
                   !                          bcondrt_g(ic,ibrt),            &
                   !                          cinfrt,cinfvs_a(jtemp),        &
...
!c                   therefore no influence)
                if (b_fluxd_mixed_bcond(ivol)) then
                  if (.not.dual_permeability) then
                    ...
                    dtotcflux(ibl) = - fluxd_dp(dtotc(ibl,tid),r0,     &
                                       bdyinfrt_da,cinfvs_a(jtemp),    &
                                       pornew(ivol),sanew(ivol),       &
                                       pornew(jvol),sanew(jvol),       &
                                       yg(ivol),yg(jvol))
...
!c diffusive as function of concentration gradients 
     ...
     &                           cinfrt)                             ! bdyinfrt_dg
                 ...
                 !dgflux = dgflux                                    & !diffusion flux
                 !       - fluxd_g_dp(dtotg(ibl),                    & 
                 !                    r0,                            &
                 !                    cinfrt,cinfvs_a(jtemp),        &
                 !                    pornew(ivol),sgnew(ivol),      &
                 !                    pornew(jvol),sgnew(jvol),      &
