...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/tprfrtlc.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
#else
      use gen, only : ilog, b_writeversion_tecplot,                    &
                      b_output_trans_binary, realbuffer_gb,            &
...
#include <finclude/petscsys.h>
      ...
      external alkcalc, comptotc, phpe, rateredx, satindex,            &
...
!c  total aqueous component concentrations 
      ...
      if (b_output_trans_binary) then
        ...
        write(iunit1,'(50e15.6e3)') time,(totc(ic),ic=1,nc-1)
      end if
...
!c  species concentrations
      ...
      if (b_output_trans_binary) then
        ...
        write(iunit2,'(1000e15.6e3)') time,(c(ic),ic=1,nc-1),  &
                                   (cx(ix),ix=1,nxout)
      end if
...
!c  master variables
     ...
     &               iax,jax,nc,nx,namec,namex)
      ...
      if (b_output_trans_binary) then
        ...
        if ((ph_output).and.(pe_output)) then
          write(iunit3,'(13e15.6e3)') time,ph,pe,eh,strion,alk_carb,   &
                                    alk_noncarb,alk_tot,alk_carb_mg,   &
                                    alk_noncarb_mg,alk_tot_mg,         &
                                    tempkel-tconv,zbal                  
        elseif ((.not.ph_output).and.(pe_output)) then                  
          write(iunit3,'(12e15.6e3)') time,pe,eh,strion,alk_carb,      &
                                    alk_noncarb,alk_tot,alk_carb_mg,   &
                                    alk_noncarb_mg,alk_tot_mg,         &
                                    tempkel-tconv,zbal                  
        elseif ((ph_output).and.(.not.pe_output)) then                  
          write(iunit3,'(11e15.6e3)') time,ph,strion,alk_carb,         &
                alk_noncarb, alk_tot,alk_carb_mg,alk_noncarb_mg,       &
...
!c  total gas pressure
        ...
        if (b_output_trans_binary) then
          ...
          write(iunit4,'(50e15.6e3)') time,(rgasatm*tempkel*g(ig),     &
     &                              ig=1,ng),pres_tot
        end if
...
!c  uvsnew was passed into hhead
          ...
          if (b_output_trans_binary) then
            ...
            write(iunit11,'(50e15.6e3)') time,(rateg(ig,tid),ig=1,ng)
          end if
...
!c  !!!rateint
      ...
      if (naq.eq.0.and.(nr.gt.0).and.(.not.redox_equil)) then
        ...
        if (b_output_trans_binary) then
          ...
          write(iunit5,'(50e15.6e3)') time,(rateor(ir,tid),ir=1,nr)
        end if
...
!c  reaction rates of intra-aqueous kinetic reactions
      ...
      if (naq.gt.0) then 
        ...
        if (b_output_trans_binary) then
          ...
          write(iunit5,'(50e15.6e3)') time,(rateaq(iaq,tid),iaq=1,naq)
        end if
...
!c  skip for far-from-equilibrium reactions
        ...
        if (b_output_trans_binary) then
          ...
          write(iunit7,'(50e15.6e3)') time,(satm_log(im),im=1,nm)
        end if
...
!c  compute porosity from mineral volume fractions for output
      ...
      if (update_porosity) then
        if (b_output_trans_binary) then
          ...
          write(iunit8,'(50e15.6e3)') time,(phi_out(im),im=1,nm),porvol
        end if
        ...
        if (b_output_trans_binary) then
          ...
          write(iunit8,'(50e15.6e3)') time,(phi_out(im),im=1,nm),por_out
        end if
...
!CMX
          ...
          call modrate(ratem,cm(im),deltat,im,tid)
        ...
        if (b_output_trans_binary) then
          ...
          write(iunit9,'(50e15.6e3)') time,(ratemp(ireac,tid),         &
                                    ireac=istart,istop)
        end if
...
!c  saturation indices (excluded minerals)
     ...
     &                             iamx,jamx,imx)
         ...
         if (b_output_trans_binary) then
           ...
           write(iunit10,'(50e15.6e3)') time,(dlog10(satmx(imx)),      &
                                      imx=1,nmx)
         end if
...
!c_isotopes    
      if (iso_output) then
        ...
        if (b_output_trans_binary) then
          ...
          write(iunit12,'(50e15.6e3)') time,(valuetemp(imi),imi=1,i1)
        end if
        
      end if

!c  compress total aqueous component concentration vector in case ofF
!c  equilibrium reactions 
