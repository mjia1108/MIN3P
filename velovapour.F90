...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/velovapour.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
!c  local variables
      ...
      real*8 aread(3,12), areax(3,12), dist(3,12), vel(3), areai,      &
             ...
             vel_jvol ,diff_energybal,tortuosity_vap,tortuosity              
...
!c  find flux between control volume pair
                ...
                if (split_divdensv) then
                   dflux = diff_energybal(uvsnew(ivol),uvsnew(jvol),r1,&
                                          r1,pornew(ivol),pornew(jvol),&
                                          sgnew(ivol),sgnew(jvol),     &
                                          cinfevap_pa(i1sav), r1,r1,   &
                                          tortuosity,ups_flow,.false.)
                   dflux = dflux + diff_energybal(tempnew(ivol),       &
                                   tempnew(jvol),r1, r1,pornew(ivol),  &
                                   pornew(jvol),sgnew(ivol),           &
                                   sgnew(jvol),cinfevap_t(i1sav),      &
                                   r1,r1,tortuosity,ups_flow,.false.)
                else
                   dflux = diff_energybal(densvnew(ivol),              &
                                         densvnew(jvol),r1,            &
                                         r1,pornew(ivol),pornew(jvol), &
                                         sgnew(ivol),sgnew(jvol),      &
                                         cinfevap_pa(i1sav),r1,r1,     &
...
!c  ouputdd routine: iprint = 1; else iprint = 0
           ...
           if (b_output_binary) then
             ...
             write(ivelvap,'(9e15.6e3)') xg_out,yg_out,zg_out,         &
