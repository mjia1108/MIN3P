...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/initopgs.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
!c ----------------------------------------------------------------------
      ...
      integer :: i, info_debug, l_string, ierr, im, istart, istop,     &
                 igb, igs, ivx, ivy, ivz, ivol, jvol, idum, itemp_vol, &
                 imb, ic, icur, ix, i1, icount, ii, iiso, inic, ilink
...
!c  exit program, if specified output time > final solution time
            ...
            do igs = 1,ngs
              if (gs_tout(igs).gt.tfinal/time_factor+small) then
                if (rank == 0) then  
                  write(ilog,*) 'ABNORMAL EXIT from initopgs-1'
...
!c  check array dimensions
            ...
            do igb = 1,ngb
              if (ngb_vol(igb).gt.nngl) then
                if (rank == 0) then  
                  write(ilog,*) 'ABNORMAL EXIT from initopgs-2'
...
!c If domain-decomposition is used, it only falls
!c in one sub-domain.
                  if (xcoord.lt.dist_x .or. xcoord.gt.rmax_x) then
                    !goto 998
                    ivx = -1
                  else
                    do ivx = nvxls,nvxle
...
!c check that ycoord falls within model domain
                  ...
                  if(ycoord.lt.dist_y .or. ycoord.gt.rmax_y) then
                    !goto 998
                    ivy = -1
                  else    
                    do ivy = nvyls,nvyle
...
!c check that zcoord falls within model domain
                  ...
                  if (zcoord.lt.dist_z .or. zcoord.gt.rmax_z) then
                    !goto 998
                    ivz = -1
                  else    
                    do ivz = nvzls,nvzle
...
!c warn user that coordinate falls on control volume boundary
                  ...
                  if(ivx > 0 .and. ivy > 0 .and. ivz > 0) then
                    ...
                    !3D domain
                    if (nvxgbl > 1 .and. nvygbl > 1 .and. nvzgbl > 1) then 
                      ngb_vol_ijface(ilink,igb) = nvxgl*nvygl*(ivz-nvzgls) +   &     
                              nvxgl*(ivy-nvygls) + ivx-nvxgls+1
                    !2D xy domain                                                  
                    else if (nvxgbl > 1 .and. nvygbl > 1) then
                      ngb_vol_ijface(ilink,igb) = nvxgl*(ivy-nvygls) + ivx-nvxgls+1
                    !2D yz domain                                                  
                    else if (nvygbl > 1 .and. nvzgbl > 1) then
                      ngb_vol_ijface(ilink,igb) = nvygl*(ivz-nvzgls) + ivy-nvygls+1
                    !2D xz domain                                                  
                    else if (nvzgbl > 1 .and. nvxgbl > 1) then
                      ngb_vol_ijface(ilink,igb) = nvxgl*(ivz-nvzgls) + ivx-nvxgls+1
                    !1D x domain                                                   
                    else if (nvxgbl > 1) then
                      ngb_vol_ijface(ilink,igb) = ivx-nvxgls+1
                    !1D y domain                                                   
                    else if (nvygbl > 1) then
                      ngb_vol_ijface(ilink,igb) = ivy-nvygls+1
                    !1D z domain                                                   
                    else if (nvzgbl > 1) then
                      ngb_vol_ijface(ilink,igb) = ivz-nvzgls+1
                    end if                    
