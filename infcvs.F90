...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/infcvs.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
!c ----------------------------------------------------------------------
      ...
      use m_heat_transport, only : heat_transport, energy_balance,     &
                                   tempnew, b_water_freezing,          &
                                   water_freezing_temper
...
!c  loop over control volumes
    ...
    !$omp areaf, cxx_i, cxx_ij, cxx_j,                                &
    !$omp cyy_i, cyy_ij, cyy_j, czz_i, czz_ij, czz_j,                 &
    !$omp delx_i, delx_j, dely_i, dely_j, delz_i, delz_j)
...
!c  assign conductivities for current control volume
            ...
            if (permeability_field) then
              ...
              cxx_i = condxx(izn)       !assign hydraulic conductivities
              cyy_i = condyy(izn)
              czz_i = condzz(izn)
...
!c  permeability update due to porosity changes
              ...
              if (b_adjust_coeff) then
                if (tempnew(ivol) < water_freezing_temper .or.         &
                    tempnew(jvol) < water_freezing_temper) then
              ...
              !!Test
              !if(jtemp == 24) then
              !    write(idbg,'(7(a,1x,e13.6,1x))') "cinfvs(jtemp)-X1", &
              !          cinfvs(jtemp), "cxx_ij",cxx_ij,"areaf",areaf,  &
              !          "cxx_i",cxx_i, "delx_j",delx_j,"cxx_j",cxx_j,  &
              !          "delx_i",delx_i
              !end if  
...
!c  permeability update due to porosity changes
               ...
               !!Test
               !if(jtemp == 24) then
               !    write(idbg,'(7(a,1x,e13.6,1x))') "cinfvs(jtemp)-X2", &
               !          cinfvs(jtemp), "cxx_ij",cxx_ij,"areaf",areaf,  &
               !          "cxx_i",cxx_i, "delx_j",delx_j,"cxx_j",cxx_j,  &
               !          "delx_i",delx_i
               !end if 
...
!c  permeability update due to porosity changes
               ...
               if (update_permeability.or.update_permeability_flow) then
                ...
                !!Test
                !if(jtemp == 24) then
                !    write(idbg,'(7(a,1x,e13.6,1x))') "cinfvs(jtemp)-Y1", &
                !          cinfvs(jtemp), "cyy_ij",cyy_ij,"areaf",areaf,  &
                !          "cyy_i",cyy_i, "dely_j",dely_j,"cyy_j",cyy_j,  &
                !          "dely_i",dely_i
                !end if 
...
!c  permeability update due to porosity changes
               ...
               if (update_permeability.or.update_permeability_flow) then
                ...
                !!Test
                !if(jtemp == 24) then
                !    write(idbg,'(7(a,1x,e13.6,1x))') "cinfvs(jtemp)-Y2", &
                !          cinfvs(jtemp), "cyy_ij",cyy_ij,"areaf",areaf,  &
                !          "cyy_i",cyy_i, "dely_j",dely_j,"cyy_j",cyy_j,  &
                !          "dely_i",dely_i
                !end if 
                ...
                if (ivyp.gt.nvygls-1) then    !front connection (4)
                  jvol = javs(jtemp)              
                  cinfvs(jtemp) = cvol(ivol)+cvol(jvol)
                  cinfvs_a(jtemp) = cinfvs(jtemp)
                  if (b_adjust_coeff) then
                    if (tempnew(ivol) < water_freezing_temper .or.       &
                        tempnew(jvol) < water_freezing_temper) then
...
!c  permeability update due to porosity changes
               ...
               if (update_permeability.or.update_permeability_flow) then
                ...
                !!Test
                !if(jtemp == 24) then
                !    write(idbg,'(7(a,1x,e13.6,1x))') "cinfvs(jtemp)-Z1", &
                !          cinfvs(jtemp), "czz_ij",czz_ij,"areaf",areaf,  &
                !          "czz_i",czz_i, "delz_j",delz_j,"czz_j",czz_j,  &
                !          "delz_i",delz_i
                !end if 
...
!c  permeability update due to porosity changes
               ...
               if (update_permeability.or.update_permeability_flow) then
                ...
                !!Test
                !if(jtemp == 24) then
                !    write(idbg,'(7(a,1x,e13.6,1x))') "cinfvs(jtemp)-Z2", &
                !          cinfvs(jtemp), "czz_ij",czz_ij,"areaf",areaf,  &
                !          "czz_i",czz_i, "delz_j",delz_j,"czz_j",czz_j,  &
                !          "delz_i",delz_i
                !end if 
      ...
      if (info_debug.gt.0 .and. b_enable_output .and.                  &
        ...
        do irow=1,nngl
          ...
          write(igen,'(6(a,i8,a,es10.3,1x))')     &
              ('cinfvs(',i1,') = ',cinfvs_a(i1),i1=istart,iend)
        end do
      end if
