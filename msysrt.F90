...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/msysrt.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
!c  write total system mass in aqueous phase to file
      if(rank == 0 .and. b_enable_output .and.                       &
        ...
        if (b_output_trans_binary) then
          ...
          write(imrt,'(40(1pe12.4))') time_io,(cmass(ic),ic=1,nc-1)
        end if
      end if
...
!c  write total system mass in aqueous phase to file
      if(rank == 0 .and. b_enable_output .and.                         &
        ...
        if (b_output_trans_binary) then
          ...
          write(imrt,'(40(1pe12.4))') time_io,(smass(imb),imb=1,nmb)
        end if
      end if
...
!c  write total system mass in gaseous phase to file
        ...
        if(rank == 0 .and. b_enable_output .and.                       &
          ...
          if (b_output_trans_binary) then
            ...
            write(imrt,'(40(1pe12.4))') time_io,(gmass(ig),ig=1,ng)
          end if
        end if
      ...
      if (.not.noncompetitive_sorption) then
        ...
        if (sorption_group.eq.'ion-exchange') then 
          if (b_output_trans_binary) then
            ...
            write(imrt,'(40(1pe12.4))') time_io, (csbmass_ion(isb),    &
                                                  isb=1,nsb_ion)
          end if  
...
!c  only non-competitive sorption
      ...
      elseif(nsb_ion.eq.0 .and. nsb_surf.eq.0) then
        if (b_output_trans_binary) then
          ...
          write(imrt,'(40(1pe12.4))') time_io,(amass(ianc),ianc=1,nanc)
        end if
...
!c  write total mineral mass in system to file
        ...
        if(rank == 0 .and. b_enable_output .and.                       &
          ...
          if (b_output_trans_binary) then
            ...
            write(imrt,'(40(1pe12.4))') time_io,(cmmass(im),im=1,nm)
          end if
        end if
