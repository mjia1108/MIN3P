...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/jacrt.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
!cdsu-------------------------------------------------------------------
     ...
     &       wfac                 
      ...
      real*8 :: acoff, bulkconc, cputime, dcstor, dgstor, drtinc,      &
                ...
                gasm, gasv, distance, fluxvg, fluxd, fluxd_dp,         &
                fluxv_vl, fluxv_vl_dp, gasdiff2, fluxd_g_dp
...
!c ms variables	
      ...
      external acoff, bulkconc, comptotc, dtotcong,             &
               fluxd, fluxv, fluxv_vl, fluxv_vl_dp, fluxd_dp,   &
               fluxd_g_dp, rateredx, rhsrt,                     &
               ...
               atotconc, ratemin, ratemin_new, aratemin,        &
               delcmigfunc, drategas, drategdd, drateint,       &
               drateint_new, dratemin, dratemin_new, draterdx,  &
               dtotconc, dtotdyvisc, gasconc, molconc, rategas, &
               rategasd, rateint, rateint_new, tcorr, totint,   &
               totredx, updtsvap

      real*8 :: r0, r1, r2, small, gsatmin, rverysmall, enat
      parameter (r0 = 0.0d0, r1 = 1.0d0, r2 = 2.0d0, small = 1.0d-10,  &
                 gsatmin=0.0d0, rverysmall = 1.0d-30,                  &
                 enat = 2.71828182845904509d0)
      
      !temp hardwired parameters for dual permeability implementation-Mehrnoush-27/11/2012
      !real*8 :: beta_dp, dist_dp, w_fracture, diff_eff_dp, w_matrix, mat_frac_face_y
      ...
      !if (dual_permeability) then
      !
      !  !cmj_dp  temp hardwired parameters for dual permeability implementation-Mehrnoush-27/11/2012
      !  beta_dp = 3.0d0         !unitless
      !  dist_dp = 0.01          !m
      !  !w_fracture = 0.05      !unitless
      !  w_fracture = 0.5        !unitless      
      !  !diff_eff_dp = 0d0      !m2/d, equal to Mass Tr=0 cm2/d in HYDRUS1D
      !  !diff_eff_dp = 0.5d-5   !m2/d, equal to Mass Tr=0.05 cm2/d in HYDRUS1D
      !  diff_eff_dp = 0.5d-4    !m2/d, equal to Mass Tr=0.5 cm2/d in HYDRUS1D
      !  !diff_eff_dp = 0.5d-03  !m2/d, equal to Mass Tr=5 cm2/d in HYDRUS1D
      !  !diff_eff_dp = 0.5d-02  !m2/d, equal to Mass Tr=50 cm2/d in HYDRUS1D
      !  
      !  !cmj_dp  precalculations for solute exchange terms-Mehrnoush-27/11/2012
      !  !cmj_dp  added alpha_s_dp_fixed to hardwire first order solute mass transfer coefficient (alfa-s),
      !  !cmj_dp  effective diffusion (Da) has taken out of formula and w_matrix is added to it
      !  !cmj_dp  alpha_s_dp_fixed will be multiplied by Da in ... subroutine-Mehrnoush-18/01/2013
      !
      !  w_matrix = r1-w_fracture
      !  
      !  !alpha_s_dp_fixed = beta_dp*diff_eff_dp /dist_dp**r2
...
!c  depletion of mineral
            if (diss_vol) then 
              ...
              if (cmnew(im,ivol).le.cmcmin(im,tid).and.ratemdp(im,ivol).le.r0) then
                ratemdp(im,ivol) = r0
              end if
            end if
     ...
     &                             cinfrt_mcd(i1))
          ...
          if (.not.dual_permeability) then 
            ...
            if ((yg(ivol) > mat_frac_face_y .and.                      &
              ...
              cflux(icon,ic) = fluxv_vl_dp(totcnew(ic,ivol),           & !advective term
                                           totcnew(ic,jvol),           &
                                           ivol,jvol,                  &
                                           cinfrt_va(i1),ic)           &
                             - fluxd_dp(totcnew(ic,ivol),              & !dispersive term
                                        totcnew(ic,jvol),              &
                                        cinfrt_da(i1),cinfvs_a(i1),    &
...
!c diffusion expressed in terms of concentration gradients
     ...
     &                                 cinfrt)
            ...
            !     yg(jvol) > mat_frac_face_y)) then
              ...
              gflux(icon,ic) = gflux(icon,ic)                            & !advective flux 
                             - fluxd_g_dp(totgnew(ic,ivol),              & !diffusive flux
                                          totgnew(ic,jvol),              &
                                          cinfrt,cinfvs_a(i1),           &
...
!c    otherwise: set derivative of rate to zero
              ...
              if (((cmnew(im,ivol)+dissvol).lt.                        &
     &            (r1+small)*cmcmin(im,tid).and.diss_vol).or.          &
...
! prc -------------------------------------------------------
     ...
     &           mdiff_ic_cvol(:,i1),mdiff_ix_cvol(:,i1), drtinc,tid)
     ...
     &                         vel_lin(i1),ivol)                      
            ...
            call delcmigfunc(electromignew(:,ivol),                   &
                             electromignew_inc(:,ivol),drtinc,tid)     
...
!cmj_dp  compute derivative of flux and derivative of total influx 
!cmj_dp  (aqueous phase)for dual-permeability-13/11/2012 
              if (.not.dual_permeability) then
                dcflux = fluxv_vl(dtotc(ibl,tid),r0,                   &    !advective term
     &                            ivol,jvol,                           &
     &                            cinfrt_va(i1),ibl)                   &
     ...
     &                         cinfrt_da(i1))
                ...
                if ((yg(ivol) > mat_frac_face_y .and.                  &
                    ...
                    (yg(ivol) < mat_frac_face_y .and.                  &
                     yg(jvol) > mat_frac_face_y)) then
                  dcflux = fluxv_vl_dp(dtotc(ibl,tid),r0,              &  !advective term
     &                                 ivol,jvol,                      &
     &                                 cinfrt_va(i1),ibl)              &
     &                   - fluxd_dp(dtotc(ibl,tid),r0,                 & !dispersive term
     &                              cinfrt_da(i1),cinfvs_a(i1),        &
     &                              pornew(ivol),sanew(ivol),          &
     &                              pornew(jvol),sanew(jvol),          &
     &                              yg(ivol),yg(jvol))
...
!c diffusive as function of concentration gradients 
     ...
     &                               cinfrt)
            ...
            !     yg(jvol) > mat_frac_face_y)) then
              ...
              dgflux = dgflux                                    &
                     - fluxd_g_dp(dtotg(ibl),                    & !diffusive flux
                                  r0,                            &
                                  cinfrt,cinfvs_a(i1),           &
                                  pornew(ivol),sgnew(ivol),      &
                                  pornew(jvol),sgnew(jvol),      &
