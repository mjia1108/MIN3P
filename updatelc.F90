...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/updatelc.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
!c ----------------------------------------------------------------------
      ...
      subroutine updatelc(c,ulc,ilog,tid,not_converged,zone_name)
...
!c suppress local chemistry output if nodal aqueous values are read for initial condition
...
!c  write convergence history to screen
     ...
     &    zone_name.ne.'temperature-correction') then
        ...
        if (iter_lc(tid).eq.1 .or. idetail_lc(tid).eq.2) then                    
          write(ilog,'(a)') ' Newton Iteration Convergence Summary: lc'   
