...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/icrtlczn.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
!c ----------------------------------------------------------------------
      ...
      integer :: i, iaq, ic, icount, im, isb, isites, ilinear, itemp,  &
                 l_string, icount1
      
      real*8 :: sitemass, sitearea, sitedens, cec_frc_total
      ...
      character*72 name, nametemp, nametemp1
      ...
      if (found_subsection) then
        ...
        do i= 1, nmin
          ...
          if (sdtype(i) .eq. 'equation') then
              ...
              if (icount .eq. 0 .or. iciunt1 .lt. nmin) then
                  if (rank == 0 .and. b_enable_output) then
...
!c read the inhibition component. Note: only one component is allowed.              
               ...
               if (icount .eq. 0 .or. iciunt1 .lt. nmin) then
                  if (rank == 0 .and. b_enable_output) then
                    write(ilog,*) 'error reading the component name in salinity dependent reaction rate of minerals'
                    exit
                  end if
              end if
        ...
        if (nlinear.gt.0) then
          ...
          do ic = 1,nc
            if (isotherm_type(ic).eq.'linear') then
              write(igen,'(a12,1x,1pe15.4)') namec(ic), distcoff_lc(ic)
            end if
          end do
        ...
        if (sorption_group.eq.'surface-complexation'.or.(nsb_surf.gt.0 &
          ...
          do ic=1,nc
            if (component_type(ic).eq.'surface') then
              isites = iais(ic)
              write(igen,'(a12,2x,3(1pe12.4,3x))') namec(ic),     &
                                                site_mass(isites),&
                                                site_area(isites),&
                                                site_dens(isites)
            end if
