...
!c ----------------------------------------------------------------------
      ...
      subroutine velocity_g_ex(l_sufx, suffix, nmax, njamxc, cinfradx,  &
                               radial_coordx)
      use gen
      use chem
      use phys
...
#include <finclude/petscsys.h>
      ...
      real*8 :: gasp_m, gasd_m, gasv, fluxvg, fluxd, xg_out, yg_out,   &
                zg_out, gpi, gasdiff2, fluxd_g_dp

      external cliqdisp
      external fluxvg, gasp_m, gasd_m, gasv, wgprop, gasdiff2, fluxd_g_dp
...
!c  version information
        ...
        if (b_output_binary) then
          ...
          tec_variables(1:9) = [character(len=72) ::                   &
                          "x", "y", "z", "vx dif", "vy dif", "vz dif", &
                          "vx tot", "vy tot", "vz tot"]
                          
          
          call tecplot_binary_write_variable(Petsc_Comm_World,         &
                       igsa2_list(ic),9, tec_variables(1:9),           &
                       offset_igsa2(ic),b_output_mpiio_single,.false.) 
        ...
        if (rank == 0 .and. b_enable_output) then
          ...
          write(ifls,'(2a)')                                           &
              '3        z                               ','m'
          write(ifls,'(2a)')                                           &
              '4        v_x Fick s diffusion            ','moles/m^2/d'
          write(ifls,'(2a)')                                           &
              '5        v_y Fick s diffusion            ','moles/m^2/d'
          write(ifls,'(2a)')                                           &
              '6        v_z Fick s diffusion            ','moles/m^2/d'
          write(ifls,'(2a)')                                           &
              '7       v_x total                       ','moles/m^2/d'
          write(ifls,'(2a)')                                           &
              '8       v_y total                       ','moles/m^2/d'
          write(ifls,'(2a)')                                           &
              '9       v_z total                       ','moles/m^2/d'
...
!cz     &                                     spt_weight            )
            ...
            if ((yg(ivol) > mat_frac_face_y .and.              &
              ...
              dgflux_cd(ic) = - fluxd_g_dp(totgnew(ic,ivol),              & !diffusion flux
                                           totgnew(ic,jvol),              &
                                           cinfrt,cinfvs_a(i1),           &
              ...
              dgflux_cd(ic) = - fluxd(totgnew(ic,ivol),              & !diffusion flux
                                      totgnew(ic,jvol),              &
                                      cinfrt)
            end if                      
...
!                                     gpi/pa_atm/) 
!            else
!              write(igsa,'(7e15.6e3)') xg_out,yg_out,zg_out,           &
!                                    (velg(idim),idim=1,3),             &
!                                     gpi/pa_atm
!            end if
!
...
!                                            gmfrac(:,ivol2))/)
!            else
!              write(igsr,'(4e15.6e3)') xg_out,yg_out,zg_out,           &
!                                     gasd_m(mdens_g(ivol2),            &
!                                            gmfrac(:,ivol2))
!            end if
!
...
!                                     gasv(gmfrac(:,ivol2))/)
!            else
!              write(igsy,'(4e15.6e3)') xg_out,yg_out,zg_out,           &
!                                     gasv(gmfrac(:,ivol2))
!            end if
!
...
!            else
!              write(igsk,'(22e15.6e3)') xg_out,yg_out,zg_out,          &
!                                  (grad_adv(idim),idim=1,3),           &
!                                  (grad_diff(ig,1),ig=1,ng),           &
!                                  (grad_diff(ig,2),ig=1,ng),           &
!                                  (grad_diff(ig,3),ig=1,ng)
...
!c           mass fluxes
            ...
            do ic=1,n
              ...
              if (b_output_binary) then
                ...
                write(igsa2,'(24e15.6e3)') xg_out,yg_out,zg_out,       &
                                   (velg_cd(ic,idim),idim=1,3),        &
                                   (velg_cd(ic,idim),idim=1,3)
              end if
            end do  
...
!c  variable diffusion coefficients
           ...
           !     else
           !       write(igsf2,'(6e15.6e3)') xg_out,yg_out,zg_out,      &
           !             (diff(ic,idim)/sec_per_days,idim=1,3) 
           !       
           !      
           !     end if
...
!          else
!            write(ihycx,'(4e15.6e3)') xg(ivol),yg(ivol),zg(ivol),      &
!                                    perm_fac(ivol)
!          end if
!        end do
!        
