...
!> $URL: https://biot.eos.ubc.ca/svn/min3p_thcm/branches/dsu_dual_permeability/src/min3p/opnpgfls.F90 $
!---------------------------------------------------------------------
!********************************************************************!
...
#include <finclude/petscsys.h>
      ...
      integer :: i, ianc, ic, ig, iaq, im, ir, ix, ivol, igsi_igsb,    &
                 ...
                 i1, ic1, ic2, icount, jvol, l_sfx2, ierr,             &
                 ivx, ivy, ivz
      
      real*8 :: xout, yout, zout, zoutput
      ...
      integer :: nvarsigbp, nvarsigfvel, nvarsigcvel, nvarsiggvel
...
!c  write header for transient data to file information file
        ...
        if (transient_flow) then
          ...
          if (b_output_binary) then
            allocate(offset_igfvel(ngb_ijface))
            offset_igfvel = 0
            allocate(offset_igfvel_ijk(ngb_ijface))
            offset_igfvel_ijk = 0
            allocate(igfvel(ngb_ijface))
            igfvel = 0
          else
            allocate(igfvel(ngb_ijface))
            igfvel = 0
        ...
        if (reactive_transport) then            
          if (b_output_binary) then
            allocate(offset_igcvel(n,ngb_ijface))
            offset_igcvel = 0
            allocate(offset_igcvel_temp(n,ngb_ijface))
            offset_igcvel_temp = 0
            allocate(offset_igcvel_ijk(n,ngb_ijface))
            offset_igcvel_ijk = 0
            allocate(igcvel(n,ngb_ijface))
            igcvel = 0
            
            !mengqi_jia
            
            if (ng.gt.0) then
              allocate(offset_iggvel(n,ngb_ijface))
              offset_iggvel = 0
              allocate(offset_iggvel_temp(n,ngb_ijface))
              offset_iggvel_temp = 0
              allocate(offset_iggvel_ijk(n,ngb_ijface))
              offset_iggvel_ijk = 0
              allocate(iggvel(n,ngb_ijface))
              iggvel = 0
            end if
        ...
        do igb = 1,ngb_ijface                 !loop over output locations
          ...
          if (ivol < 0 .or. jvol < 0) then   !control volume in the current domain
            cycle 
          end if    !End control volume in the current domain
              ...
              if (b_output_binary) then
                ...
                if (b_output_multizone) then
                  call tecplot_binary_write_zoneinfo(                          &
                               Petsc_Comm_World, igcvel(ic,igb),               &
...
#ifdef PETSC
                write(igcvel(ic,igb),'(4(a,i5,1x),a)')                         &
...
#else           
                write(igcvel(ic,igb),'(2(a,i5,1x),a)')                         &
...
! gas exchange flux (mengqi jia)    
            
            if (ng.gt.0) then
            
            if (b_output_binary) then
              nvarsiggvel = 7  
              tec_variables(1:nvarsiggvel) = [character(len=72) ::              &
                                "time", "vx dif", "vy dif", "vz dif", "vx tot", &
                                "vy tot","vz tot"]
            end if
            ...
            do ic = 1, n
              call icnvrt(0,ic,suffix2,l_sfx2,ierr)
!c  open file
              iggvel(ic,igb) = lun_get()
              if (b_output_trans_binary) then
                call binary_file_open(PETSC_COMM_SELF,                 &
                             iggvel(ic,igb), prefix(:l_prfx)//'_'//&
                             suffix(:l_sufx)//'_'//suffix2(:l_sfx2)//  &
                             '.ggvel', .true.)
              else
                open(iggvel(ic,igb),file=prefix(:l_prfx)//'_'//        &
                     suffix(:l_sufx)//'_'//suffix2(:l_sfx2)//'.ggvel', &
                     status='unknown',form='formatted')
              end if
...
!c  write file information
              if (rank == 0 .and. b_enable_output) then   
                ...
                write(ifls,'(2a)')                        &
                    '1        time                           ',time_unit
                write(ifls,'(2a)')                        &
                    '2        v_x Diffusion                   ','moles/m^2/d'
                write(ifls,'(2a)')                        &
                    '3        v_y Diffusion                   ','moles/m^2/d'
                write(ifls,'(2a)')                        &
                    '4        v_z Diffusion                   ','moles/m^2/d'
                write(ifls,'(2a)')                        &
                    '5       v_x Total                       ','moles/m^2/d'
                write(ifls,'(2a)')                        &
                    '6       v_y Total                       ','moles/m^2/d'
                write(ifls,'(2a)')                        &
                    '7       v_z Total                       ','moles/m^2/d'
                
              end if  

!c  version information
              if (b_writeversion_tecplot .and. .not. b_output_binary) then
                call writeversion2file(iggvel(ic,igb), "#")
              end if
              
              if (b_output_binary) then
                strbuffer = 'dataset '//prefix(:l_prfx)//'' 
                offset_iggvel(ic,igb) = 0
                call tecplot_binary_write_header(Petsc_Comm_World,             &
                             iggvel(ic,igb),"#!TDV102", trim(strbuffer),       &
                             offset_iggvel(ic,igb),b_output_mpiio_single,.false.) 
                
                call tecplot_binary_write_variable(Petsc_Comm_World,           &
                             iggvel(ic,igb),7, tec_variables(1:7),             &
                             offset_iggvel(ic,igb),b_output_mpiio_single,.false.)
                
                strbuffer = 'diffusion fluxes'
                offset_iggvel_temp(ic,igb) = offset_iggvel(ic,igb)
                
                if (b_output_multizone) then  !!check
                  call tecplot_binary_write_zoneinfo(                          &
                               Petsc_Comm_World, iggvel(ic,igb),               &
                               trim(strbuffer),offset_iggvel(ic,igb),          &
                               itec_vel,jtec_vel,ktec_vel,                     &
                               b_output_mpiio_single,.false.,b_output_multizone)
                  offset_iggvel(ic,igb) = offset_iggvel_temp(ic,igb) +         &
                                (11+len_trim(strbuffer))*4*nprcs + 4
                else
                  call tecplot_binary_write_zoneinfo(                          &
                               Petsc_Comm_World, iggvel(ic,igb),               &
                ...
                offset_iggvel_ijk(ic,igb) = offset_iggvel_temp(ic,igb) - 5*4
              else
                write(iggvel(ic,igb),'(3a)') 'title = "dataset ',              &
                                             prefix(:l_prfx),'"'
                write(iggvel(ic,igb),'(5a)') 'variables = "time", ',           &
                                  '"vx dif", "vy dif", "vz dif", ',            &
                                  '"vx tot", "vy tot", "vz tot"'
#ifdef PETSC
                write(iggvel(ic,igb),'(4(a,i5,1x),a)')                         &
                   'zone t = "diffusion fluxes, local volume = ',&
                    ivol,', global volume = ', node_idx_lg2g(ivol),            &       
                   'connected to local volume = ',jvol,                        &
                   ', global volume = ', node_idx_lg2g(jvol),'", f=point'
#else           
                write(iggvel(ic,igb),'(2(a,i5,1x),a)')                         &
                   'zone t = "diffusion fluxes, volume = ',ivol, &
                   'connected to volume = ',jvol,'", f=point'
#endif          
              end if
